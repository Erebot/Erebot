 ...
 *              pointed by \a var, one of the cases will be used.
 *              The page at http://unicode.org/cldr/data/charts/supplemental/language_plural_rules.html
 *              references all the available forms par language.</td>
 *      </tr>
 *  </table>
 */
class   ErebotStyling
{
    ...
    public function __construct($source, iErebotI18n &$translator)
    {
        ...
        $this->dom->relaxNGValidate($schema);
        libxml_clear_errors();
        ...
        if (count($errors)) {
            ...
            throw new EErebotInvalidValue('Error while validating the message');
        }
        ...
        // Handle plurals.
            ...
            for (   $child = $node->firstChild;
                ...
                if ($child->nodeType != XML_ELEMENT_NODE ||
                    $child->tagName != 'case')
                    continue;
                // See http://unicode.org/cldr/data/charts/supplemental/language_plural_rules.html
                // for a list of the supported forms for each language.
                $form = $child->getAttribute('form');
                $subcontents[$form] = $this->parseNode($child, $attributes, $variables);
                $pattern .= $form.'{'.$form.'} ';
            }
            $pattern .= '}';
