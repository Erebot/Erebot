...
extends ErebotConfigProxy
{
    ...
    public function load($configData, $source)
    {
        ...
        $schema = dirname(__FILE__).'/config.xsd';
        ...
        if (count($errors)) {
            foreach ($errors as $error) {
                switch ($error->code) {
                    ...
                    case 1757:
                        fprintf(STDERR, "No schema found to validate the configuration file. Did you run 'make' ?");
            ...
            throw new EErebotInvalidValue(
                'Unknown error while validating the configuration file');
        }
