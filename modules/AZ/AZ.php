...
extends ErebotModuleBase
{
    ...
    public function reload($flags)
    {
        if ($flags & self::RELOAD_HANDLERS) {
            ...
            $this->handlers['create']   =   new ErebotEventHandler(
                                                array($this, 'handleCreate'),
                                                'ErebotEventTextChan',
                                                NULL, $filter);
            $this->connection->addEventHandler($this->handlers['create']);

            $targets = new ErebotEventTargets(ErebotEventTargets::ORDER_ALLOW_DENY);
            $filter = new ErebotTextFilter(ErebotTextFilter::TYPE_REGEXP, AZ::WORD_FILTER);
            $this->handlers['rawText']  =   new ErebotEventHandler(
    ...
    public function handleCreate(iErebotEvent &$event)
    {
        ...
        $targets =& $this->handlers['rawText']->getTargets();
        $targets->addRule(
