...
{
    protected $tv;
    protected $default_mapping = NULL;
    ...
    public function reload($flags)
    {
        ...
        if ($flags & self::RELOAD_MEMBERS) {
            ...
            $this->tv = $class::getInstance();

            $config         =&  $this->connection->getConfig($this->channel);
            $moduleConfig   =&  $config->getModule($this->moduleName);
            ...
            foreach ($groups as $param) {
                ...
                $this->custom_mappings[$group] =
                    array_map('trim', explode(',', $chans));
            }

            try {
                $this->default_group = $this->parseString('default_group');
                if (!isset($this->custom_mappings[$this->default_group]))
    ...
    public function handleTv(iErebotEventMessageText &$event)
    {
        ...
        if (rtrim($channels) == '') {
            if ($this->default_group)
                $channels   = $this->custom_mappings[$this->default_group];
            else {
