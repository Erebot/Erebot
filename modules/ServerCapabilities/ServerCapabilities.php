...
extends ErebotModuleBase
{
    ...
    {
        return isset($this->supported['ELIST']);
    }
    ...
    public function hasCommand($cmdName)
    {
        ...
        if (isset($this->supported['CMDS']) &&
            in_array($cmdName, $this->supported['CMDS']))
            return TRUE;
        return FALSE;
    }
    ...
    public function hasStatusMsg($status)
    {
        ...
        if (isset($this->supported['STATUSMSG']) &&
            is_string($this->supported['STATUSMSG'])) {
    ...
    public function isChannel($chan)
    {
        ...
        $prefix     = $chan[0];
        $allowed    =  (isset($this->supported['CHANTYPES']) &&
                        is_string($this->supported['CHANTYPES'])) ?
                        $this->supported['CHANTYPES'] : '#&';
    ...
    public function getMaxListSize($list)
    {
        ...
        switch ($list) {
            ...
            case self::LIST_INVITES:
                if (isset($this->supported['MAXBANS']) &&
            ...
            case self::LIST_SILENCES:
                if (isset($this->supported['SILENCE']) &&
                    ctype_digit($this->supported['SILENCE']))
                    return (int) $this->supported['SILENCE'];
                throw new EErebotNotFound($translator->gettext(
                    'No silence limit specified'));

            case self::LIST_WATCHES:
                if (isset($this->supported['WATCH']) &&
                    ctype_digit($this->supported['WATCH']))
    ...
    public function getChanLimit($chanPrefix)
    {
        ...
        if (isset($this->supported['MAXCHANNELS']) &&
            ctype_digit($this->supported['MAXCHANNELS']))
    ...
    public function getMaxTextLen($type)
    {
        ...
        switch ($type) {
            case self::TEXT_AWAY:
                if (isset($this->supported['AWAYLEN']) &&
                    ctype_digit($this->supported['AWAYLEN']))
                    return (int) $this->supported['AWAYLEN'];
                break;

            case self::TEXT_CHAN_NAME:
                if (isset($this->supported['CHANNELLEN']) &&
                    ctype_digit($this->supported['CHANNELLEN']))
                    return (int) $this->supported['CHANNELLEN'];
                return 200;

            case self::TEXT_KICK:
                if (isset($this->supported['KICKLEN']) &&
                    ctype_digit($this->supported['KICKLEN']))
                    return (int) $this->supported['KICKLEN'];
                break;

            case self::TEXT_NICKNAME:
                if (isset($this->supported['NICKLEN']) &&
                    ctype_digit($this->supported['NICKLEN']))
                    return (int) $this->supported['NICKLEN'];
                return 9;

            case self::TEXT_TOPIC:
                if (isset($this->supported['AWAYLEN'])) {
                    if (ctype_digit($this->supported['AWAYLEN']))
                        return (int) $this->supported['AWAYLEN'];
                }
                return -1;
    ...
    {
        if (isset($this->supported['CASEMAPPING']) &&
            is_string($this->supported['CASEMAPPING']))
    ...
    public function irccasecmp($a, $b, $mappingName = NULL)
    {
        ...
        if (!isset(self::$caseMappings[$mappingName]))
            throw new EErebotNotFound($translator->gettext(
                'No such mapping exists'));
        $mapping = self::$caseMappings[$mappingName];
    ...
    {
        if (isset($this->supported['CHARSET']) &&
            is_string($this->supported['CHARSET']))
    ...
    {
        if (isset($this->supported['NETWORK']) &&
            is_string($this->supported['NETWORK']))
    ...
    public function getChanModeForPrefix($prefix)
    {
        ...
        $ok = preg_match(self::PATTERN_PREFIX, $this->supported['PREFIX'], $matches);
        if ($ok) {
    ...
    public function qualifyChannelMode($mode)
    {
        ...
        if (!isset($this->supported['CHANMODES']) ||
            !is_array($this->supported['CHANMODES']))
    ...
    public function getMaxTargets($cmd)
    {
        ...
        else if (isset($this->supported['MAXTARGETS']) &&
    ...
    public function getMaxListModes($modes)
    {
        ...
        if (!isset($this->supported['MAXLIST']))
            return $this->getMaxVariableModes();

        foreach ($this->supported['MAXLIST'] as $maxs => $limit) {
            $maxs = str_split($maxs);
    ...
    {
        if (isset($this->supported['MAXPARA']) &&
            ctype_digit($this->supported['MAXPARA']))
    ...
    public function getSSL()
    {
        if (isset($this->supported['SSL'])) {
            ...
            if (is_array($this->supported['SSL']))
                return $this->supported['SSL'];
        }
    ...
    public function supportsStandard($standard)
    {
        ...
        if (isset($this->supported['STD'])) {
            ...
            else if (is_array($this->supported['STD']))
                $standards = $this->supported['STD'];
    ...
    {
        if (is_array($this->supported['EXTBAN']) &&
            isset($this->supported['EXTBAN'][1])) {
            return $this->supported['EXTBAN'][1];
        }
